...
import { lifecycleApi } from '../services/analyticsApi'

const CACHE_TTL = 5 * 60 * 1000 // 5 minutes

/**
 * useLifecycleAnalytics - Fetch and cache content lifecycle analytics
 * 
 * Handles loading, error, and success states for lifecycle data.
 ...
 */
export function useLifecycleAnalytics(options = {}) {
  const {
  ...
  // Fetch lifecycle analytics data
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await lifecycleApi.get(timePeriod, startDate, endDate)
      cacheRef.current = { data: result, timestamp: Date.now() }
      setData(result)
      setLastRefreshTime(new Date())
    } catch (err) {
      setError(err.message || 'Failed to fetch lifecycle analytics')
      console.error('Lifecycle analytics fetch error:', err)
    } finally {
      setLoading(false)
    }
