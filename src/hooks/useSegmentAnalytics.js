...
import { segmentsApi } from '../services/analyticsApi'

const CACHE_TTL = 5 * 60 * 1000 // 5 minutes

/**
 * useSegmentAnalytics - Fetch and cache audience segment analytics
 * 
 * Handles loading, error, and success states for segment data.
 ...
 */
export function useSegmentAnalytics(options = {}) {
  const {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await segmentsApi.get(timePeriod, startDate, endDate)
      cacheRef.current = { data: result, timestamp: Date.now() }
      setData(result)
      setLastRefreshTime(new Date())
    } catch (err) {
      setError(err.message || 'Failed to fetch segment analytics')
      console.error('Segment analytics fetch error:', err)
    } finally {
      setLoading(false)
    }
