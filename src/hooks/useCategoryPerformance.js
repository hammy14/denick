...
import { categoriesApi } from '../services/analyticsApi'

const CACHE_TTL = 5 * 60 * 1000 // 5 minutes

/**
 * useCategoryPerformance - Fetch and cache category performance analytics
 * 
 * Handles loading, error, and success states for category data.
...
export function useCategoryPerformance(options = {}) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await categoriesApi.get(timePeriod, startDate, endDate)
      cacheRef.current = { data: result, timestamp: Date.now() }
      setData(result)
      setLastRefreshTime(new Date())
    } catch (err) {
      setError(err.message || 'Failed to fetch category performance')
      console.error('Category performance fetch error:', err)
    } finally {
      setLoading(false)
    }
