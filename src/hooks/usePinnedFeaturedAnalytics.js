...
import { pinnedFeaturedApi } from '../services/analyticsApi'

const CACHE_TTL = 5 * 60 * 1000 // 5 minutes

/**
 * usePinnedFeaturedAnalytics - Fetch and cache pinned/featured content analytics
 * 
 * Handles loading, error, and success states for pinned/featured data.
 ...
 */
export function usePinnedFeaturedAnalytics(options = {}) {
  const {
  ...
  // Fetch pinned/featured analytics data
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await pinnedFeaturedApi.get(timePeriod, startDate, endDate)
      cacheRef.current = { data: result, timestamp: Date.now() }
      setData(result)
      setLastRefreshTime(new Date())
    } catch (err) {
      setError(err.message || 'Failed to fetch pinned/featured analytics')
      console.error('Pinned/featured analytics fetch error:', err)
    } finally {
      setLoading(false)
    }
